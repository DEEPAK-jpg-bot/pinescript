...
type ChatStore = {
    ...
    createConversation: (title?: string, optimistic?: boolean) => Promise<string | null>;
...
export const useChatStore = create<ChatStore>((set, get) => ({
    ...
    createConversation: async (title?: string, optimistic: boolean = false) => {
        const user = get().user;
        if (!user) return null;
    ...
    sendMessage: async (content: string) => {
        console.log('--- SendMessage Protocol Started ---');
        const { user, activeConversationId, checkRateLimit, setError, createConversation } = get();

        const sanitized = sanitizeInput(content);
        if (!sanitized) {
            console.log('Aborting: Empty or invalid input');
            return;
        }

        if (!user) {
            console.log('Aborting: No user session found');
        ...
        try {
            ...
            console.log('Step 2: Quota Verification');
            const rateCheck = await checkRateLimit();
            if (rateCheck && !rateCheck.allowed) {
                console.log('Quota Blocked:', rateCheck.reason);
                throw new Error(rateCheck.reason === 'daily_quota_exceeded'
            ...
            if (!conversationId) {
                console.log('Step 3: Initializing New Logic Session');
                conversationId = await createConversation(sanitized.slice(0, 30) + '...', true);
                if (!conversationId) throw new Error("Failed to initialize logic session.");
                console.log('New Session ID:', conversationId);
            }

            // 3. Database Sync (User Message)
            console.log('Step 4: Database Synchronization');
            ...
            console.log('DB Persist Success, MsgID:', userMessage.id);
